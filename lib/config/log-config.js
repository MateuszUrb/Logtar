...
class LogConfig {
  ...
  static from_file(file_path) {
    ...
    const bufferToStr = file_contents.toString("utf-8");
    return LogConfig.from_json(JSON.parse(bufferToStr));
  }

  /**
   * @typedef {object} JsonLogConfig
   * @property {string} [level] current log level
   * @property {RollingConfig} [rolling_config] a current instance of RollingConfig
   * @property {string} [file_prefix] currently applying file prefix
   */

  /**
   * @param {JsonLogConfig} json The json object to be parsed into {LogConfig}.
  ...
  with_file_prefix(file_prefix) {
    if (typeof file_prefix !== "string") {
      throw new Error(
        `file_prefix must be a string. Unsupported param ${JSON.stringify(file_prefix)}`,
      );
    }
